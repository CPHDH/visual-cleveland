...
function doLineChart(
  container,
  comparison_csv,
  includeTotalPopulations = false,
  includeCityPopulations = true
) {
  ...
  d3.csv(comparison_csv, function (data) {
    ...
    var city_data_charting = cityChartingData(
      city_years,
      city_population_total,
      city_population_black
    );
    // suburbs data
    ...
    var suburbs_data_charting = cityChartingData(
      suburbs_years,
      suburbs_population_total,
      suburbs_population_black
    );
    ...
    var maxScale = includeTotalPopulations
      ? d3.max(suburbs_population_total)
      : includeCityPopulations
      ? d3.max(city_population_black)
      : d3.max(suburbs_population_black);
    var yScale = d3.scaleLinear().domain([0, maxScale]).range([height, 0]);
    ...
    if (includeCityPopulations) {
      ...
      var cityblack = svg
        ...
        .attr("id", "path-black-city")
        .attr("d", lineGeneratorBlack)
        .attr("stroke-dasharray", "3 3");

      setTimeout(() => {
        var cityblack_label = svg
          .append("text")
          .attr("dy", -6)
          .attr("dx", 6)
          .append("textPath")
          .attr("class", "text-path")
          .attr("href", "#path-black-city")
          .text(city_name + " - Black");
      }, 1000);

      // duplicate (line mask)
      var cityblack_mask = svg
        .append("path")
        .datum(city_data_charting)
      ...
      cityblack_mask
        .attr(
          "stroke-dasharray",
          cityblack_mask.node().getTotalLength() +
            " " +
            cityblack_mask.node().getTotalLength()
        )
      ...
      if (includeTotalPopulations) {
        ...
        var citytotal = svg
          ...
          .attr("id", "path-total-city")
          .attr("d", lineGeneratorTotal)
          .attr("stroke-dasharray", "3 3");

        setTimeout(() => {
          var citytotal_label = svg
            .append("text")
            .attr("dy", -6)
            .attr("dx", 6)
            .append("textPath")
            .attr("class", "text-path")
            .attr("href", "#path-total-city")
            .text(city_name + " - total");
        }, 1000);

        // duplicate (line mask)
        var citytotal_mask = svg
          .append("path")
          .datum(city_data_charting)
          .attr("class", "line-mask city")
          .attr("d", lineGeneratorTotal);
        ...
        citytotal_mask
          .attr(
            "stroke-dasharray",
            citytotal_mask.node().getTotalLength() +
              " " +
              citytotal_mask.node().getTotalLength()
          )
          .attr("stroke-dashoffset", 0)
    ...
    var suburbsblack = svg
      .append("path")
      .datum(suburbs_data_charting)
      .attr("class", "line-black suburbs")
      .attr("id", "path-black-suburbs")
      .attr("d", lineGeneratorBlack);

    setTimeout(() => {
      var suburbsblack_label = svg
        .append("text")
        .attr("dy", -6)
        .attr("dx", 6)
        .append("textPath")
        .attr("class", "text-path")
        .attr("href", "#path-black-suburbs")
        .text("Suburbs - Black");
    }, 1000);
    ...
    suburbsblack
      .attr(
        "stroke-dasharray",
        suburbsblack.node().getTotalLength() +
          " " +
          suburbsblack.node().getTotalLength()
      )
    ...
    if (includeTotalPopulations) {
      var suburbstotal = svg
        .append("path")
        .datum(suburbs_data_charting)
        .attr("class", "line-total suburbs")
        .attr("id", "path-total-suburbs")
        .attr("d", lineGeneratorTotal);

      setTimeout(() => {
        var suburbstotal_label = svg
          .append("text")
          .attr("dy", -6)
          .attr("dx", 6)
          .append("textPath")
          .attr("class", "text-path")
          .attr("href", "#path-total-suburbs")
          .text("Suburbs - total");
      }, 1000);
      ...
      suburbstotal
        .attr(
          "stroke-dasharray",
          suburbstotal.node().getTotalLength() +
            " " +
            suburbstotal.node().getTotalLength()
        )
    ...
    /*
		...
		**************
		*/
    //     var broken = true; // :LOL, will fix
    //     if (!broken) {
    //       var textOffsetX = (coords) => {
    //         return coords + 5;
    //       };
    //       var textOffsetY = (coords) => {
    //         return coords + 5;
    //       };
    //
    //       var suburbs_total_coords = suburbstotal
    //         ? [
    //             suburbstotal._groups[0][0].pathSegList[years_covered.length - 1].x,
    //             suburbstotal._groups[0][0].pathSegList[years_covered.length - 1].y,
    //           ]
    //         : null;
    ...
    //       }
    //
    //       var city_total_coords = citytotal
    //         ? [
    //             citytotal._groups[0][0].pathSegList[years_covered.length - 1].x,
    //             citytotal._groups[0][0].pathSegList[years_covered.length - 1].y,
    //           ]
    //         : null;
    ...
    //           .attr("opacity", 1);
    //       }
    //       var suburbs_black_coords = suburbsblack
    //         ? [
    //             suburbsblack._groups[0][0].pathSegList[years_covered.length - 1].x,
    //             suburbsblack._groups[0][0].pathSegList[years_covered.length - 1].y,
    //           ]
    //         : null;
    ...
    //       }
    //
    //       var city_black_coords = cityblack
    //         ? [
    //             cityblack._groups[0][0].pathSegList[years_covered.length - 1].x,
    //             cityblack._groups[0][0].pathSegList[years_covered.length - 1].y,
    //           ]
    //         : null;
    ...
    ui_totals.onchange = function (e) {
      ...
      doLineChart(
        container,
        comparison_csv,
        includeTotalPopulations,
        includeCityPopulations
      );
    };
    var ui_totals_label = document.createElement("label");
    ui_totals_label.setAttribute("for", "includeTotals");
    ui_totals_label.innerHTML =
      "Include Total (Black and Non-Black) Populations for Selected View <span></span><span></span>";
    ...
    ui_city_label.innerHTML =
      "Include " + city_name + " (Non-Suburban) Black Population <span></span>";
...
function doDataTable(
  ...
  container,
  years_covered,
  summaryLabel = "Total for all suburbs",
  highlightControls = true,
  highlightPopulationMax = false,
  highlightPercentageMax = false
) {
  d3.select(container).html(""); // clear container
    ...
    .data(function (row) {
      return columns.map(function (column, i) {
        if (typeof row[column] === "object") {
          ...
          var b =
            row[column].population_black !== null
              ? row[column].population_black
              : "no data";
          var t =
            row[column].population_total !== null
              ? row[column].population_total
              : "no data";
          var b_int =
            row[column].population_black !== null
              ? row[column].population_black
          ...
          v =
            ...
            row[column].percentage_black +
            "%" +
            '<br><span class="fraction" >' +
            (b !== t ? b + "/" + t : b) +
            "</span></div>";

          // update column totals
          column_totals[total_position].total =
            column_totals[total_position].total + t_int;
          column_totals[total_position].black =
            column_totals[total_position].black + b_int;
        } else {
  ...
  column_totals.forEach(function (obj) {
    ...
    cell_data +=
      ...
      percent_sum +
      '<br><span class="fraction">' +
      population_black_sum +
      "/" +
      population_total_sum +
      "</span></div></td>";
  });
    ...
    years_covered.forEach(function (y) {
      ...
      if (highlightPercentageMax) {
        let pm = document.querySelector(
          "table div._" + y + '[data-percent="' + percent_max + '"]'
        );
      ...
      if (highlightPopulationMax) {
        let bm = document.querySelector(
          "table div._" + y + '[data-black="' + black_max + '"]'
        );
        bm.classList.add("max-black");
        if (bm.classList.contains("max-percent")) {
          bm.setAttribute(
            "data-title",
            "Highest percentage & largest black population, " + y
          );
        } else {
  ...
  if (highlightControls) {
    ...
    ui_population_max.onchange = function (e) {
      ...
      doDataTable(
        ...
        summaryLabel,
        highlightControls,
        highlightPopulationMax,
        highlightPercentageMax
      );
    };
    var ui_population_max_label = document.createElement("label");
    ui_population_max_label.setAttribute("for", "popmax");
    ui_population_max_label.innerHTML =
      "Highlight <span>largest black population</span> per census year";
    ...
    ui_percentage_max_label.innerHTML =
      "Highlight <span>highest percentage black</span> per census year";
  ...
  data.forEach(function (row) {
    years_covered.forEach(function (y) {
      ...
      row[y] = {
        ...
        percentage_black:
          percentage !== null ? round(percentage * 100, 2) : "--",
      };
    });
    calculated.push(row);
  });
