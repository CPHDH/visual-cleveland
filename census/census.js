...
function addCalculatedRows(rows) {
  rows.forEach(function (row, i) {
    ...
    row.change_percent =
      typeof percentage(row) == "number" ? percentage(row) : 0;
...
function doBarChart(csv, container, span) {
  ...
  var xScale = d3.scale.ordinal().rangeRoundBands([0, width], 0.075);
  ...
  d3.csv(csv, function (d) {
    ...
    var bars = chart
      ...
      .attr("fill", function (d, i) {
        return colors(+d.population);
      })
      .attr("class", function (d) {
        return +d.change_sum < 0 ? "bar negative" : "bar positive";
      })
      .attr("x", function (d) {
...
function doDivergingChart(csv, container, span) {
  ...
  var y = d3.scale.ordinal().rangeRoundBands([0, height], 0.075);
  ...
  d3.csv(csv, function (data) {
    ...
    var bars = chart
      ...
      .transition()
      .attr("x", function (d) {
        return x(Math.min(0, +d.change_sum));
      })
      .attr("width", function (d) {
        return Math.abs(x(+d.change_sum) - x(0));
      })
      .duration(500)
      .delay(100);
    ...
    var labels = chart
      ...
      .attr("class", function (d) {
        return +d.change_sum < 0 ? "label negative" : "label positive";
      })
      .attr("height", y.rangeBand())
      .attr("y", function (d) {
        return y(d.year) + y.rangeBand() * 0.66;
      })
      .attr("x", function (d) {
        var neg = x(0) + 5;
        var pos = x(0) - 30;
        return +d.change_sum < 0 ? neg : pos;
      });
...
function doPieChart(csv, container, as_percentages) {
  ...
  d3.csv(csv, function (d) {
    ...
    var r = width / 2;
    var color = d3.scale.ordinal().range(["#ccc", "#c61c6f", "#ffb832"]);
    console.log(color);

    var data = [
      { label: "Nuetral", value: neutral },
